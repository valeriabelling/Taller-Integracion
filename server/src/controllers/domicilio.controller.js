...
const getDom = async(req, res) => {
    const response = await pool.query("SELECT * FROM domicilio ORDER BY iddomicilio ASC");
...
const createDom = async(req, res) => {
    const { idusuario, cp, localidad, calle, numero, descripcion } = req.body;
    const response = await pool.query("INSERT INTO iddomicilio (idusuario, cp, localidad, calle, numero, descripcion) VALUES ($1, $2, $3, $4, $5, $6)", [idusuario, cp, localidad, calle, numero, descripcion]);
    res.json({
        message: "Se ha agregado correctamente el domicilio.",
        body: {
            user: { idusuario, cp, localidad, calle, numero, descripcion }
        }
    });
};
