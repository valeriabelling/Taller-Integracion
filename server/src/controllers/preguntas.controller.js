...
const getPreg = async(req, res) => {
    const response = await pool.query(
        "SELECT * FROM pregunta ORDER BY idpregunta ASC"
    );
...
const getPregById = async(req, res) => {
    ...
    const response = await pool.query(
        "SELECT * FROM pregunta WHERE idpregunta = $1", [idpregunta]
    );
...
const createPreg = async(req, res) => {
    ...
    const response = await pool.query(
        "INSERT INTO pregunta (idcomprador, idvendedor, idherramienta, pregunta, respuesta) VALUES ($1, $2, $3, $4, $5)", [idcomprador, idvendedor, idherramienta, pregunta, respuesta]
    );
    res.json({
